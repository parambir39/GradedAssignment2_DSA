...
public class Node {
	...
	class Maintree
	{
		...
		static void BTSSkewed(Node root,
									int order)
		{
			...
			if(order > 0)
			{
				BTSSkewed(root.right, order);
			}
			else
			{
				BTSSkewed(root.left, order);
			}
			...
			if (order > 0)
			{
				BTSSkewed(leftNode, order);
			}
			else
			{
				BTSSkewed(rightNode, order);
			}
		}
